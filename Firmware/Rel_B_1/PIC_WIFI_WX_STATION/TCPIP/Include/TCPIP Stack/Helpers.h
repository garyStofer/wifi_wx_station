...
extern DWORD	LFSRSeedRand(DWORD dwSeed);
extern WORD	LFSRRand(void);
extern DWORD	GenerateRandomDWORD(void);
//extern char *itoa(char *buffer, int i);
...
extern BOOL	StringToIPAddress(BYTE* str, IP_ADDR* IPAddress);
extern BYTE 	ReadStringUART(BYTE* Dest, BYTE BufferLen);
...
extern char * 	strnchr(const char *searchString, size_t count, char c);
...
extern DWORD   swapl(DWORD v);

extern WORD    CalcIPChecksum(BYTE* buffer, WORD len);
...
extern void FormatNetBIOSName(BYTE Name[16]);
...
extern BYTE ExtractURLFields(BYTE *vURL, PROTOCOLS *protocol, BYTE *vUsername, WORD *wUsernameLen, BYTE *vPassword, WORD *wPasswordLen, BYTE *vHostname, WORD *wHostnameLen, WORD *wPort, BYTE *vFilePath, WORD *wFilePathLen);
extern SHORT Replace(BYTE *vExpression, ROM BYTE *vFind, ROM BYTE *vReplacement, WORD wMaxLen, BOOL bSearchCaseInsensitive);
