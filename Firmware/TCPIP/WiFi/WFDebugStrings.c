...
static void WF_OutputSecurityDebugMessage(const APP_CONFIG *p);
#endif
...
#if !defined(__18CXX)
static void WF_OutputSecurityDebugMessage(const APP_CONFIG *p_appcfg)
{
    BYTE  temp[4];
    char tmp;
    ROM char * SecurityModes[] = {"None","WEP 40","WEP 104","WPA+Key","WPA+Phrase","WPA2+Key","WPA2+Phrase","WPA1&2+Key","WPA1&2+Phrase" };

    putrsUART("Security mode:   ");
    tmp = p_appcfg->SecurityMode;
    if (tmp >= sizeof(SecurityModes)-1)
       tmp =sizeof(SecurityModes)-1;

    putrsUART( SecurityModes[tmp]);
    putrsUART("\r\n");
    
    if (p_appcfg->SecurityMode == WF_SECURITY_WEP_40 || p_appcfg->SecurityMode == WF_SECURITY_WEP_104 )
    {
        uitoa(p_appcfg->WepKeyIndex, temp);
        putrsUART("WEP Key Index:   ");
        putrsUART(temp);putrsUART("\r\n");
    }else if (p_appcfg->SecurityMode == WF_SECURITY_WPA_WITH_PASS_PHRASE ||
