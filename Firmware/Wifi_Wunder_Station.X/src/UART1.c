...
// TODO: is this necessary ? -- I don't think so as long as the uart is in charge it dictates the state of the port
 //	UART2TX_TRIS = 0;
 //	UART2RX_TRIS = 1;

    	U1MODE = 0x8000;	// Set UARTEN to 8n1.  Note: this must be done before setting UTXEN
...
// immediately sends the buffer out on the port

void
UART1_PutS(char *buffer)
{

    /* transmit until NULL character is encountered */
...
UART1_GetStringLen( void )
{
    ...
    while (ndx != wr_ndx)   // while we have charcaters in the buffer
    {
        ...
        if(c == 0 )         // the null character
        {
            if (n == 0 )    // this is s null string -- skip it, thus eliminate it from the buffer
            {
                rd_ndx = ndx;
                continue;
            }
 ...
 * Returns the number of characters read from the UART input ring buffer.
...
UART1_GetString ( char * s)
{
    ...
    do
    {
        ...
        {
            s[0] = '\0';         // return with a NULL string and 0 characters read
            return 0;
        }
         n++;
        ndx++;
...
_U1RXInterrupt(void)
{
    ...
    // Copy the byte into the ring buffer and replace newlines with null terminators for strings
    ...
    if ( wr_ndx !=   (unsigned char)  (rd_ndx -1)  ) // as long as there is room in the buffer
    {
        ...
        RX_buff[wr_ndx] = 0;  // the buffer wrapped around -- incoming characters are being lost
                              // put a null into the boundary byte and don't increment any further --
